...
struct fy_atom *
fy_emit_token_comment_handle(struct fy_emitter *emit, struct fy_token *fyt, enum fy_comment_placement placement) {
...
static int
do_buffer_output(struct fy_emitter *emit, enum fy_emitter_write_type type, const char *str, int len, void *userdata) {
...
static int
do_file_output(struct fy_emitter *emit, enum fy_emitter_write_type type, const char *str, int len, void *userdata) {
    FILE *fp = userdata;
...
static int fy_emit_streaming_node(struct fy_emitter *emit, struct fy_eventp *fyep, int flags) {
    ...
    switch (fye->type) {
        ...
        case FYET_SEQUENCE_START:
            ...
            fy_emit_common_node_preamble(emit, fye->sequence_start.anchor, fye->sequence_start.tag, emit->s_flags,
                                         emit->s_indent);
        ...
        case FYET_MAPPING_START:
            ...
            fy_emit_common_node_preamble(emit, fye->mapping_start.anchor, fye->mapping_start.tag, emit->s_flags,
                                         emit->s_indent);
